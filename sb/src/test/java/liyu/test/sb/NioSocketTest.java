...
/**
 * @author liyu
 * 此示例从网上找的，说明bio和nio的区别：
 * 1，bio是基于流的，nio是基于块的
 * 2，bio会阻塞，需要启动相应的线程及时的处理，而nio是异步非阻塞，事件驱动。
 * 			nio主要有：buffer，channel，selector
 * 			buffer相当于块儿，可读可写。
 * 			channel相当于流，SocketChannel可以设置configureBlocking(false)为非阻塞，FileChannel不可以。
 * 			selector是多路复用选择器，将channel注册到selector上，调用selectctor.select()就可以处理读写事件。
 */
public class NioSocketTest {
	private int ports[];
	private ByteBuffer echoBuffer = ByteBuffer.allocate(1024);

	public NioSocketTest(int ports[]) throws IOException {
		this.ports = ports;
		go();
	}
